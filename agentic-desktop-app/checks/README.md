...
template + its `composesWith` closure composes via
`@papercusp/template-kit` `composeTemplates` — pins consistent, exactly one
app scope, and the **full additive union-of-checks** (`templateId:checkId`)
survives. That union IS the app's definition of done.
...
The union rule means a composed app must ALSO pass the aspect templates'
suites: `confinement-guard` / `seam-round-trip` / `gym-signals`
(`../../papercusp-ops-hives/checks/`) and `boot-e2e`
(`../../tauri-desktop-shell/checks/`). Worked example config for consumer #1:
